...
public class BookProvider extends ContentProvider {
    ...
    private Uri insertBook(Uri uri, ContentValues values) {
        String name = values.getAsString(BookContract.BookEntry.COL_BOOK_NAME);
        if (name == null) {
            throw new IllegalArgumentException("This field cannot be empty");
        }

        String author = values.getAsString(BookContract.BookEntry.COL_BOOK_AUTH);
        if (author == null) {
            author = getContext().getResources().getString(R.string.UNKNOWN);
        }

        Integer pages = values.getAsInteger(BookContract.BookEntry.COL_BOOK_PAGES);
